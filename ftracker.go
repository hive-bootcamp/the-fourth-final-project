...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	uspeed := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(uspeed, 2)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
