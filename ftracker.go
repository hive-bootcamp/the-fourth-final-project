...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	secspeed := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + ((math.Pow(secspeed, 2))/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
